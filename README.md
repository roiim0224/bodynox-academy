...
| 스튜디오 사진 (최대 10장) | `studio-<slug>-01.jpg` ~ `-10.jpg` | 가로 1600×900 / 세로 900×1600 |

### 센터 사진 일괄 등록

`~/Desktop/BPM 홈페이지/센터사진/<폴더명>/` 에 아래처럼 넣고 스크립트를 실행하면 끝입니다.

```
센터사진/광화문/1.JPG ~ 10.JPG        ← 스튜디오 사진 (번호 순서대로 슬라이더에 배치)
센터사진/광화문/광화문 메인 이미지.png   ← 파일명에 '메인' 포함 → 카드 썸네일
```

```bash
python3 tools/import-center-photos.py 광화문 gwanghwamun   # 사진 변환
python3 tools/build-centers.py                            # 페이지 반영
```

폴더명 ↔ slug: `서울역`=seoul · `광화문`=gwanghwamun · `서초`=seocho ·
`강북`=gangbuk · `자카르타`=jakarta · `방콕`=bangkok

변환 스크립트가 처리하는 것:
- 스튜디오 사진 가로 1600px, 품질 62 (장당 150~300KB)
- 메인 이미지 1000×1000, 품질 72
- **아이폰 EXIF 회전 태그를 픽셀에 반영하고 태그를 1로 초기화** (그대로 두면 브라우저에서 돌아간 채 표시됨)
- 세로 사진은 슬라이더에서 잘리지 않도록 전체 표시

사진을 `assets/img/` 에 넣고 `python3 tools/build-centers.py` 를 실행하면
해당 센터의 슬라이더가 사진 자리에서 실제 사진으로 자동 교체됩니다.
가로·세로가 섞여 있어도 되며, 세로 사진은 잘리지 않고 전체가 보입니다.

**아이폰 사진 주의** — EXIF 회전 태그가 붙어 있으면 브라우저에서 돌아간 채로 보입니다.
변환할 때 `sips -r 90` 으로 회전을 픽셀에 반영하고 orientation 태그를 1로 되돌려야 합니다.
